...
.
### Mes choix de modules (AHMED)
Pour l’agent local, j’ai fait le choix d’utiliser plusieurs bibliothèques Python standards et reconnues afin de garantir un fonctionnement fiable, portable et efficace de la supervision. Le module psutil est utilisé car il constitue une référence pour la récupération des métriques système (CPU, mémoire, disque et réseau) de manière précise et multiplateforme, ce qui est essentiel pour un agent de supervision destiné à fonctionner sur différentes machines. La bibliothèque requests a été choisie pour gérer les communications HTTP avec le serveur central, car elle simplifie considérablement l’envoi de requêtes REST (POST/GET), tout en offrant une bonne lisibilité du code et une gestion robuste des erreurs réseau. Le module time est indispensable pour contrôler l’intervalle d’envoi des métriques et pour calculer correctement les débits réseau en fonction du temps écoulé, ce qui permet d’éviter une surcharge inutile du serveur tout en conservant une supervision régulière. Enfin, le module socket est utilisé afin d’identifier automatiquement la machine sur laquelle l’agent est exécuté, notamment grâce à la récupération du nom d’hôte et de l’adresse IP locale, ce qui facilite l’identification des agents côté serveur sans configuration manuelle complexe. L’ensemble de ces imports a été sélectionné dans une logique de simplicité et de performance en s’appuyant sur des bibliothèques standards ou largement utilisées dans le monde professionnel, garantissant ainsi un agent local léger, maintenable et facilement extensible pour de futures évolutions du système de supervision sahant que des modules comme requests et time etaient bien abordées en cours avec le professeur 

### Le fonctionnement de l'agent local
Cet agent de supervision est un programme Python conçu pour s’exécuter en continu sur une machine afin de surveiller son état et transmettre régulièrement des informations à un serveur central. Dès son démarrage, l’agent identifie la machine grâce à son nom d’hôte (hostname) et utilise des identifiants prédéfinis pour s’authentifier auprès du serveur via une requête HTTP. Cette authentification permet d’obtenir un token de sécurité, généralement de type JWT, qui sera utilisé pour sécuriser toutes les communications suivantes entre l’agent et le serveur.

Une fois authentifié, l’agent initialise les mécanismes nécessaires au calcul du débit réseau. Pour cela, il récupère les statistiques réseau globales de la machine, notamment le nombre total d’octets envoyés et reçus depuis le démarrage du système. Ces valeurs serviront de référence pour comparer les mesures futures et calculer un débit réseau en mégabits par seconde en fonction du temps écoulé entre deux relevés.

L’agent entre ensuite dans une boucle infinie de supervision. À chaque itération, il collecte plusieurs métriques système essentielles. Il mesure l’utilisation du processeur, le pourcentage de mémoire vive utilisée, l’espace disque occupé sur la partition principale, ainsi que la température du système lorsque celle-ci est disponible. En parallèle, il détermine l’adresse IP locale de la machine afin de permettre une identification réseau claire côté serveur. Le débit réseau est calculé dynamiquement en comparant les nouvelles statistiques réseau avec les précédentes, ce qui permet d’obtenir une estimation réaliste de l’activité réseau sur l’intervalle de temps considéré.

Une fois toutes les métriques collectées, l’agent les regroupe dans une structure de données au format JSON et les envoie au serveur via une requête HTTP sécurisée. Le token d’authentification est inclus dans les en-têtes de la requête afin que le serveur puisse vérifier l’identité de l’agent et autoriser la réception des données. Si l’envoi est effectué avec succès, l’agent attend un intervalle de temps prédéfini avant de recommencer un nouveau cycle de collecte et d’envoi.

Dans le cas où le serveur refuse les données, notamment si le token d’authentification a expiré ou est devenu invalide, l’agent détecte cette situation grâce au code de réponse HTTP. Il supprime alors le token actuel et tente automatiquement de se réauthentifier sans intervention humaine. Si la réauthentification réussit, l’agent reprend immédiatement l’envoi des métriques, ce qui garantit une continuité du service de supervision.
